...
namespace BDDoc.IntegrationTest
{
    ...
    public class ScenarioIntegration2Tests
    {
        ...
        private const string StoryId = "ScenarioIntegration2";
        ...
        private const string CustomScenarioText = "My custom scenario text.";
        ...
        public void CreatePlainScenario_UsingTheObjectExtensions_ANewPlainScenarioHasToBeInstanced()
        {
            ...
            Assert.AreEqual(1, itemsElement1.Elements().Count());
            var itemElement = itemsElement1.Elements().First();
            Assert.IsTrue(itemElement.Name.LocalName.Equals(Constants.CDataStoreItemElement));
            Assert.IsTrue(itemElement.Attributes().ElementAt(0).Name == Constants.CDataStoreKeyAttribute);
            Assert.IsTrue(itemElement.Attributes().ElementAt(0).Value == typeof(CustomScenarioAttribute).Name.Remove(14));
            Assert.IsTrue(itemElement.Attributes().ElementAt(1).Name == Constants.CDataStoreTextAttribute);
            Assert.IsTrue(itemElement.Attributes().ElementAt(1).Value == CustomScenarioText);
